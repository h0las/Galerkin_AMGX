...
void CSRMatrix::set(int i, int j, float aa)
{
	...
	if (na == _na) {
		float	* ta = (float*)malloc((na + CSRMatrix::DELTA)*sizeof(float));
		int		* tj = (int*)malloc((na + CSRMatrix::DELTA)*sizeof(int));
		memcpy(ta, a, na*sizeof(float));
	...
	int ii = ia[i];
	memcpy(&a[ii + 1], &a[ii], (na - ii)*sizeof(float));
...
float CSRMatrix::get(int i, int j)
{
	for (int k = ia[i]; k < ia[i+1]; k++)
	{
...
void CSRMatrix::add(int i, int j, float aa) {
	set(i, j, get(i,j)+aa);
}
